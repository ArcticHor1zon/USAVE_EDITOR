...
impl<W: Write> UnparseTo<W> for NaiveTime {
    fn unparse_to(self, writer: &mut W) -> Result<(), io::Error> {
        ...
        let ticks = secs * 10_000_000 + (self.nanosecond() as i64 / 100);
        writer.unparse(ticks)
    }
}

impl<W: Write> UnparseTo<W> for NaiveDateTime {
    fn unparse_to(self, writer: &mut W) -> Result<(), io::Error> {
        let unix_seconds = self.and_utc().timestamp();
        let unix_nano = self.and_utc().timestamp_subsec_nanos() as i64;
        let unix_ticks = unix_seconds * 10_000_000 + (unix_nano / 100);
        let ticks = unix_ticks + 621_355_968_000_000_000;
        writer.unparse(ticks)
    }
}
