...
impl<R: Read> ParseFrom<R> for ValueWithCode {
    fn parse_from(reader: &mut R) -> Result<Self, ParseError> {
        ...
        Ok(Self(reader.parse_typed(primitive_type)?))
    }
}
...
impl<R: Read> ParseFrom<R> for StringValueWithCode {
    fn parse_from(reader: &mut R) -> Result<Self, ParseError> {
        let type_byte = reader.parse::<u8>()?;
        if type_byte != BinaryType::String as u8 {
            return Err(ParseError::IoError(io::Error::new(
                io::ErrorKind::InvalidData,
                "expected String binary type",
            )));
        }
...
impl<R: Read> ParseFrom<R> for ArrayOfValueWithCode {
    fn parse_from(reader: &mut R) -> Result<Self, ParseError> {
        ...
        Ok(Self(reader.parse_sized(length as usize)?))
    }
}
