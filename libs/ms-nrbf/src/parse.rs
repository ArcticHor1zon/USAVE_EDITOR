...
impl<R: Read> ParseFrom<R> for NaiveTime {
    fn parse_from(reader: &mut R) -> Result<Self, ParseError> {
        let ticks = (reader.parse::<i64>()?).unsigned_abs();
        let total_seconds = ticks / 10_000_000;
        let nano = ((ticks % 10_000_000) * 100) as u32;
        let sec = (total_seconds % 60) as u32;
        let min = ((total_seconds / 60) % 60) as u32;
        let hour = ((total_seconds / 3600) % 24) as u32;

        NaiveTime::from_hms_nano_opt(hour, min, sec, nano).ok_or(ParseError::InvalidTimeSpan)
    }
}

// .NET DateTime stores ticks (100ns intervals) since 0001-01-01
// We convert to Unix timestamp for chrono
const TICKS_PER_SECOND: i64 = 10_000_000;
const DOTNET_UNIX_DIFF_TICKS: i64 = 621_355_968_000_000_000; // ticks from 0001-01-01 to 1970-01-01

impl<R: Read> ParseFrom<R> for NaiveDateTime {
    fn parse_from(reader: &mut R) -> Result<Self, ParseError> {
        let ticks = reader.parse::<i64>()?;
        let kind = ticks & 0xC000000000000000u64 as i64; // DateTimeKind bits
        let _ = kind; // ignore kind for now
        let raw_ticks = ticks & 0x3FFFFFFFFFFFFFFF; // clear kind bits

        let unix_ticks = raw_ticks - DOTNET_UNIX_DIFF_TICKS;
        let unix_seconds = unix_ticks / TICKS_PER_SECOND;
        let unix_nano = ((unix_ticks % TICKS_PER_SECOND) * 100) as u32;

        NaiveDateTime::from_timestamp_opt(unix_seconds, unix_nano)
